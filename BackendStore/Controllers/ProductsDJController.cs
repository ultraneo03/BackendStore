...
namespace BackendStore.Controllers
{
    ...
    public class ProductsDJController : ControllerBase
    {
        [HttpGet()]
        public async Task<ActionResult<IProductsPagination>> GetAllAsync(int page = 1, string? search = null, int size = 50, string? order = null)
        {
            ...
            List<Product> items = await DummyJsonClient.Instance.getProducts(page, search, size, order);
        ...
        {
            Product product = await DummyJsonClient.Instance.getProductDetailDyID(id);
            if (product != null)
            {
                return Ok(product);
            }
